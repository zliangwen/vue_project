<template>
  <div id="register" style="background:#fff;">
    ...
    <div class="content">
      <p class="title">欢迎加入</p>
            ...
            <input type="tel" v-model="userData.userPhone" placeholder="请输入手机号">
				...
				<div class="tips">
					<input type="checkbox" v-model="isCheck"> 我已同意<span @click="rule">《注册协议》</span>
				</div>
				<div class="submit">
          <mt-button type="primary" size="large" v-if="showTrue"  @click="register">确定</mt-button>  
          <mt-button type="primary" size="large" style="background:#999" v-if="!showTrue">正在注册</mt-button>                  
        </div>
    </div>
    <!-- mask弹窗  -->
